...
Factor1Pose3d_diff::Factor1Pose3d_diff(const Mat4 &observation, std::shared_ptr<Node> &n1,
             const Mat6 &obsInf, DiffFactor::robustFactorType robust_type):
             DiffFactor(6,6, robust_type), Tobs_(observation), W_(obsInf), J_(Mat6::Zero())
{
    // Ordering here is not a problem, the node is unique
    neighbourNodes_.push_back(n1);
}

Factor1Pose3d_diff::Factor1Pose3d_diff(const SE3 &observation, std::shared_ptr<Node> &n1,
...
void Factor1Pose3d_diff::evaluate_residuals()
{
...
void Factor1Pose3d_diff::evaluate_jacobians()
{
...
void Factor1Pose3d_diff::print() const
{
