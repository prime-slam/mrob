 ...
 * outputs:
 *    -original_to_ordered_index: the order index mapping (original index -> sorted position)
 *    -jacobian_indexes: index position in the Jacobian matrix for each original element (by reference)
 * 
